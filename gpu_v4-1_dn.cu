...
__global__ void conwayKernel(const uint32_t* __restrict__ current, uint32_t* __restrict__ next, int packed_width, int height, int coarsen)
{
    ...
    for (int c = 0; c < coarsen; c++)
    {
        ...
        uint32_t eq3 = ~eights & ~fours & twos & ones;
        uint32_t eq4 = ~eights & fours & ~twos & ~ones;
        uint32_t eq6 = ~eights & fours & twos & ~ones;
        uint32_t eq7 = ~eights & fours & twos & ones;
        uint32_t eq8 = eights & ~fours & ~twos & ~ones;

        uint32_t survive = eq3 | eq4 | eq6 | eq7 | eq8;
        uint32_t born = eq3 | eq6 | eq7 | eq8;
